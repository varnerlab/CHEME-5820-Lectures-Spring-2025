...
function _solve(model::MyBinaryBanditGreedyAlgorithmModel; 
    T::Int = 0, world::Function = _null, 
    context::AbstractBanditAlgorithmContextModel = MyEmptyContextModel())::Array{Float64,2}
    ...
    S = model.S;
    F = model.F;
    rewards = zeros(Float64, T, 4); # rewards for each arm
    θ = zeros(Float64, K); # probability for each arm
    ...
    for t ∈ 1:T
        ...
        if (p ≤ ϵₜ)
            ...
            # ok, so we need to compute the best possible arm by looking at the success and failure
            for i ∈ 1:K
                θ[i] = Beta(1+S[i], 1+F[i]) |> d-> rand(d);
            end
            aₜ = argmax(θ); # select the best possible action 
        end
...
function _solve(model::MyExploreFirstAlgorithmModel; 
    T::Int = 0, world::Function = _null, 
    context::AbstractBanditAlgorithmContextModel = MyEmptyContextModel())::Array{Float64,2}
...
function _solve(model::MyEpsilonGreedyAlgorithmModel; 
    T::Int = 0, world::Function = _null, 
    context::AbstractBanditAlgorithmContextModel = MyEmptyContextModel())::Array{Float64,2}
...
function _solve(model::MyUCB1AlgorithmModel; 
    T::Int = 0, world::Function = _null, 
    context::AbstractBanditAlgorithmContextModel = MyEmptyContextModel())::Array{Float64,2}
...
"""
function solve(model::AbstractBanditAlgorithmModel; 
    T::Int = 0, world::Function = _null, 
    context::AbstractBanditAlgorithmContextModel = MyEmptyContextModel())::Array{Float64,2}
